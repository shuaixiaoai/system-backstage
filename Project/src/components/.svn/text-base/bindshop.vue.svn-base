  ...
  <div class="bindshop">
    <el-form :inline="true" :model="formInline" class="demo-form-inline each-bindshop" v-for="(item, index) in bindArr" :key="item.shopId">
      ...
      <el-form-item label="店铺选择" label-width="100px" class="item-list">
          <el-select v-model="item.shopId" placeholder="请选择店铺">
      ...
      <el-form-item label="截止时间" label-width="100px" class="item-list">
          <el-date-picker type="date" placeholder="选择日期" v-model="item.deadline" style="width: 100%;"></el-date-picker>
      </el-form-item>
      <el-form-item label="排序" label-width="100px" class="item-list">
          <el-input v-model="item.sortBy" class="banner-input" placeholder="默认值为0"></el-input>
      </el-form-item>
    </el-form>
...
export default {
  ...
  mounted () {
    ...
    // this.bindArr.push({shopId: '', deadline: '', sortBy: '', isShow: ''})
