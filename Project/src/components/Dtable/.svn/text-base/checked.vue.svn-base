<template>
    <div class="">
      <div class="operate-wrapper">
          ...
          <el-button @click="setOrderToSentOrder()" type="info">设为已配送订单</el-button>
      </div>
      <el-table
      ref="multipleTable"
      :data="tableData"
  ...
  export default {
    ...
    methods: {
      ...
      handleCurrentChange (page) {
        var that = this
        var url = '/order/order/orderlist?shopId=10002&page=' + page + '&orderStatus=30'
      ...
      setOrderToSentOrder () {
        this.$confirm('此操作将设置该订单为已确认订单，是否继续？', '提示', {
          ...
          if (this.multipleSelection.length === 0) {
            ...
            api.setOrderToSent(postData).then(({data}) => {
              // console.log(data)
              if (data.code === 0) {
