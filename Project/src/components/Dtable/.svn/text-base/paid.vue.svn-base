<template>
    <div class="paid">
      <div class="operate-wrapper">
          <el-button @click="setOrderToChecked()" type="warning">设为已确认订单</el-button>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.loading = true
      var url = '/order/order/orderlist?shopId=10002&page=1&orderStatus=20'
    ...
    methods: {
      ...
      handleCurrentChange (page) {
        // 分页点击事件， 回去对应分页的数据
        var that = this
        var url = '/order/order/orderlist?shopId=10002&page=' + page + '&orderStatus=20'
      ...
      viewOrderDetail (index, row) {
        this.$router.push({path: '/ordersManage/Detail?', query: {orderId: row[index].order_id}})
      },
