<template>
  <div class="goods-bind-shop">
    <div class="section-bind">
      <span class="wrapper-header">绑定店铺：<span class="tip">(店铺绑定最好不要绑定同一家店铺，后台已作过滤！)</span></span>
...
export default {
  ...
  mounted () {
    ...
    let getArr = []
    for (let i = 0; i < goodDetail.shops.length; i++) {
      getArr[i] = {
        shopId: goodDetail.shops[i].shop_id,
        storage: parseInt(goodDetail.shops[i].storage),
        goodsStatus: parseInt(goodDetail.shops[i].goods_status),
        goodsCommend: parseInt(goodDetail.shops[i].goods_commend)
      }
    }
    this.bindArr = this.bindArr.concat(getArr)
    this.goodsId = JSON.parse(utils.storage.getItem('goodsId'))
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    submitForm () {
      let validate = false
      if (this.bindArr.length === 0) {
        this.$message.info('该商品为绑定店铺， 请选择添加绑定店铺， 或者放回店铺列表！')
        validate = true
      } else {
      ...
      if (validate) {
        ...
        this.updateGoodBindShop(sendData)
      }
    },
    initPostData () {
      let sendData = {}
