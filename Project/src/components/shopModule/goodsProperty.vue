  ...
  <div class="property">
      ...
      <div class="add-property">
          ...
          <div class="properties">
              <div class="" v-if="addList.length === 0" class="non-content">
                  用户可以点击新增商品属性按钮新增已有商品属性中没有的属性！
              </div>
              <div class="property-item" v-for="(item, index) in addList">
                  ...
                  <div class="item-property-value" v-for="(subItem, index) in item.specList">
          ...
          <el-button type="primary" @click="submitForm()" class="btn-create" :disabled="allDisable">立即创建</el-button>
      </div>
  </div>
</template>
  ...
  methods: {
      ...
      if (this.addList.length === 0) {
        ...
        for (let i = 0; i < this.addList.length; i++) {
          if (this.addList[i].spec_group === '') {
            ...
            for (let j = 0; j < this.addList[i].specList.length; j++) {
              if (this.addList[i].specList[j].spec === '') {
                this.$message.error('属性值为必填项！')
                return
              }
              if (this.addList[i].specList[j].price === '') {
                this.$message.error('价格为必填项！')
                return
              } else if (!utils.isNum(this.addList[i].specList[j].price)) {
    ...
    initPostData () {
      ...
      for (let i = 0; i < this.addList.length; i++) {
        ...
        for (let j = 0; j < this.addList[i].specList.length; j++) {
          sendData.addList[i].specList[j].default1 = this.addList[i].specList[j].default1 === true ? 1 : 0
          sendData.addList[i].specList[j].sortBy = this.addList[i].specList[j].sort_by
        }
    ...
    addPropertiesValue (item) {
      item.specList.push({spec: '', price: '', default1: '1', sort_by: 0})
    },
